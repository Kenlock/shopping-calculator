...
class _TipCalculatorPageState extends State<TipCalculatorPage> {
  ...
  static const defaultSalesPercentage = 0;
  ...
  final _salesPercentageController =
  TextEditingController(text: defaultSalesPercentage.toString());
  ...
  int _salesPercentage = defaultSalesPercentage;
  ...
  void initState() {
    ...
    _salesPercentageController.addListener(_onSalesAmountChanged);
  ...
  _onSalesAmountChanged() {
    setState(() {
      _salesPercentage = int.tryParse(_salesPercentageController.text) ?? 0;
    });
  }
  ...
  _getSalesAmount() => _billAmount * _salesPercentage / 100;


  //This method is used to calculate the latest total amount
  _getTotalAmount() => _billAmount - _getTipAmount()+_getSalesAmount();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Shopping Calculator', style: TextStyle(color: Colors.black87),),
        centerTitle: true,
        elevation: 0.0,
